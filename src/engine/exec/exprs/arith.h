...
std::vector<T> ArithLoop(ArithOp op, const std::vector<T> &l, const std::vector<T> &r) {
	...
	std::vector<T> out(n);
	switch (op) {
		case ArithOp::Add: for (std::size_t i = 0; i < n; ++i) out[i] = l[i] + r[i]; break;
		case ArithOp::Sub: for (std::size_t i = 0; i < n; ++i) out[i] = l[i] - r[i]; break;
		case ArithOp::Mul: for (std::size_t i = 0; i < n; ++i) out[i] = l[i] * r[i]; break;
		case ArithOp::Div:
